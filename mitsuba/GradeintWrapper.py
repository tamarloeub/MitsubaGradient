...
for case in range(n_cases):
    ...
    for nps in range(len(Np_vector)):
        for bb_gt in range(len(beta_gt_factor)):
            ...
            scene_gt[0].create_new_scene(beta=beta_gt, g=0, origin=sensors_pos[0]['origin'], target=sensors_pos[0]['target'], up=sensors_pos[0]['up'], nSamples=Np_vector[nps])
            ...
            scene_gt[1]    = pyScene()
            scene_gt[1].create_new_scene(beta=beta_gt, g=0, origin=sensors_pos[1]['origin'], target=sensors_pos[1]['target'], up=sensors_pos[1]['up'], nSamples=Np_vector[nps])
            scene_gt[2]    = pyScene()
            scene_gt[2].create_new_scene(beta=beta_gt, g=0, origin=sensors_pos[2]['origin'], target=sensors_pos[2]['target'], up=sensors_pos[2]['up'], nSamples=Np_vector[nps])
            scene_gt[3]    = pyScene()
            scene_gt[3].create_new_scene(beta=beta_gt, g=0, origin=sensors_pos[3]['origin'], target=sensors_pos[3]['target'], up=sensors_pos[3]['up'], nSamples=Np_vector[nps])
            ...
            for bb in range(len(beta0_diff)):
                ...
                for iteration in range(max_iterations):               
                    ...
                    for ss in range(n_sensors):
                        ...
                        algo_scene[ss].create_new_scene(beta=beta, g=0, origin=sensors_pos[ss]['origin'], target=sensors_pos[ss]['target'], up=sensors_pos[ss]['up'], nSamples=Np_vector[nps])
                    ...
                    #if (iteration == 0) and (in_iter < int(1 / (1 - beta1))):
                        ## initialize first ans second moment
                        #in_iter += 1
                        #continue
                    #else:
