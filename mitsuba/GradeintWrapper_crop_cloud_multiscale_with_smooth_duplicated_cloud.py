...
#if crop_f and enlarged_f:  
    #ns            = 8    
    #enlarge_scale = 3.
    #beta_gt = zoom(jpl_full_cloud[12:12+ns, 14:14+ns, 10:10+ns], enlarge_scale)
    #beta_gt = np.pad(beta_gt, pad_width=npad, mode='constant', constant_values=0.01)
    #mask    = zoom(load_mask[12:12+ns, 14:14+ns, 10:10+ns], enlarge_scale)
    #mask    = np.pad(mask, pad_width=npad, mode='constant', constant_values=0)
    #mask[mask < 0]          = 0
    #beta_gt[beta_gt < 0.01] = 0.01
    #[ nx, ny, nz ]          = beta_gt.shape      

#elif crop_f and not enlarged_f:
...
#elif enlarged_f and not crop_f:
    #enlarge_scale = 3.
    #beta_gt = zoom(jpl_full_cloud, enlarge_scale)
    #beta_gt = np.pad(beta_gt, pad_width=npad, mode='constant', constant_values=0.01)
    #mask    = zoom(load_mask, enlarge_scale)
    #mask    = np.pad(mask, pad_width=npad, mode='constant', constant_values=0)
    #mask[mask < 0]          = 0
    #beta_gt[beta_gt < 0.01] = 0.01
    #[ nx, ny, nz ]          = beta_gt.shape 
...
grid_ms  = np.array([ nx/4., nx/8., nx/16., 1. ]) # 
n_stages = len(grid_ms)
iters_ms = np.zeros(n_stages, dtype=int)

# optimizer parameters - ADAM
alpha       = 0.3#0.08 #15 # randomly select alpha hyperparams -> r = -a * np.random.rand() ; alpha = 10**(r) - randomly selected from a logarithmic scale
alpha_scale = 1#1.2#(0.15/0.5)**(1. / (n_stages - 1))
...
for ll in range(len(lambda_factor)):
    ...
    out_name += str(n_unknowns) + ' unknowns ' + str(n_sensors) + ' sensors all above the medium 1 cycle ' + str(n_pixels) + ' pixels '
    out_name += str(Np_vector[0]) + ' photons adam alpha changes with scale with space carving mask scalable Np per stage' + ' No PRIOR'
    ...
    for bb in range(len(beta0_diff)):   
        ...
        beta  = np.ones( np.array(beta_gt.shape / grid_ms[0], dtype=int) ) * beta0_diff[bb]
        ...
        for iteration in range(max_iterations):               
            ...
            for ss in range(n_sensors):
                ...
                algo_scene[ss].create_new_scene(beta=np.copy(beta), g=0.85, origin=sensors_pos[ss]['origin'], target=sensors_pos[ss]['target'], 
                                                up=sensors_pos[ss]['up'], nSamples=Np_vector[0] / Np_ms*Np_scale, sensorType='perspective', 
                ...
                #
                tmp        =  (-1) * ( zoom(I_algo[ss, bb, iteration], np.array(I_gt[ss].shape, dtype=float)/n_pixels_h ) - I_gt[ss] ) #I_gt_ms[ss])
            ...
            #if stage == n_stages - 1:        
                #smooth_grad[bb, iteration, ind0x_ms[stage], ind0y_ms[stage], ind0z_ms[stage]] = 0.0
            ...
            if (up_scale >= cost_window_size + 1) and (stage is not (n_stages - 1)):
                ...
                sio.savemat(out_path + out_name + ' start of stage ' + str(stage) + '.mat', 
                            { 'beta0_diff' : beta0_diff[bb], 'mask' : mask, 'beta_gt' : beta_gt, # Scene pre-fixed params
                              ...
                              'betas_s3' : betas_stage3, 'cost_grad_s0' : cost_grad_stage0, 'cost_grad_s1' : cost_grad_stage1, 
                              'cost_grad_s2' : cost_grad_stage2, 'cost_grad_s3' : cost_grad_stage3, 'mb' : mb, 
                              'betas_zoom_out' : betas_zoom_out, 'alpha_scale' : alpha_scale,    # Multiscale
