...
for ll in range(len(lambda_factor)):
    ...
    out_name  = '19_07_26_13:16/fractal dim stages ' + str(n_stages) + ' lambda ' + str(slambda) + ' with air and ocean ' + str(grid_size)
    out_name += ' grid points ' + str(n_unknowns) + ' unknowns ' + str(n_sensors) + ' sensors ' + str(n_pixels) + ' pixels ' + str(Np_vector[0])
    out_name += ' photons alpha and photons changes with scale space carving mask' + ' No PRIOR'

    # load results 
    load_name = out_name
    load_res = sio.loadmat(out_path + load_name + ' start of stage 4.mat')
    ...
    for bb in range(len(beta0_diff)):   
        ...
        if 'betas_s0' in load_res.keys():
            ...
            cost_grad_stage0 = load_res['cost_grad_s0']   
            smooth_grad_s0   = load_res['smooth_g_s0']   # All are set together
        else:
            betas_stage0     = 0
            cost_grad_stage0 = 0
            smooth_grad_s0   = 0
        ...
        if 'betas_s1' in load_res.keys():
            ...
            smooth_grad_s1   = load_res['smooth_g_s1']
        else:
            betas_stage1     = 0
            cost_grad_stage1 = 0
            smooth_grad_s1   = 0
        ...
        if 'betas_s2' in load_res.keys():
            ...
            smooth_grad_s2   = load_res['smooth_g_s2']       
        else:
            betas_stage2     = 0
            cost_grad_stage2 = 0
            smooth_grad_s2   = 0
        ...
        if 'betas_s3' in load_res.keys():
            ...
            smooth_grad_s3   = load_res['smooth_g_s3']  
        else:
            betas_stage3     = 0
            cost_grad_stage3 = 0
            smooth_grad_s3   = 0
