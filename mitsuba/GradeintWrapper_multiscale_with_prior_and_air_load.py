...
if flags.air_model is False:
    ...
    slambda_v = np.array([0.01])#, 0.01, 0.01, 0.01]) #, 0.01, 0, 0.01, 0.1, 0.25])
    mlambda_v = np.array([0.01])# 0.1,  0.001, 0, 0.01]) #01,   0, 0, 0.01, 0.1, 0.25])
else: 
    slambda_v = np.array([1])
...
for ll in range(len(slambda_v)):
    ...
    #out_name  = '/monoz/smooth/21_04_27_10:49/'
    #out_name  = '/smooth/21_04_27_23:33/'	
    #out_name  = '/21_04_28_14:17/'
    #out_name  = '/monoz/smooth/21_04_29_09:59/'
    #out_name  = '/monoz/smooth/21_04_30_16:27/'
    #out_name  = '/monoz/smooth/21_05_01_05:56/'
    #out_name += 'monoz/smooth/21_04_21_12:47/monoz/21_04_21_13:59/monoz/smooth/21_04_21_15:49/monoz/21_04_21_18:37/smooth/21_04_21_21:43/'

    #out_name  = '/monoz/smooth/21_05_26_10:55/'
    #out_name  = '/monoz/smooth/21_05_27_01:41/'
    #out_name  = '/smooth/21_05_27_12:05/'
    out_name  = '/monoz/smooth/21_05_27_23:02/'
	
    paths.append(flags.out_path + out_name)
    
    if not os.path.exists(flags.out_path + out_name):
        os.makedirs(flags.out_path + out_name)
        shutil.copyfile(os.path.realpath(__file__), flags.out_path + out_name + 'exe_script.py')
    ...
    for bb in range(len(beta0_diff)):
        ...
        # load results 
        load_res = sio.loadmat(flags.out_path + out_name + ' iter.mat')
        #load_res = sio.loadmat(flags.out_path + out_name + ' start of stage ' + str(n_stages- 2) + '.mat')
        ...
        cost_data[:, :prev_iter] = load_res['cost_data'][:, :prev_iter]
        cost_mean[:, :prev_iter] = load_res['cost_mean'][:, :prev_iter]
        smooth_term[:, :prev_iter] = load_res['smooth_term'][:, :prev_iter]
        monoZ_term[:, :prev_iter]  = load_res['monoZ_term'][:, :prev_iter]

        # update lamdbas
        slambda = load_res['smooth_lambda'][0,0]
        mlambda = load_res['monoz_lambda'][0,0]
        ...
        cost_window_size = 10 
        last_stage_iter  = iters_ms[stage - 1] 
        mb[:, :prev_iter]       = load_res['mb'][:, :prev_iter]
        mass_err[:, :prev_iter] = load_res['mass_err'][:, :prev_iter]	
        ...
        beta_tmp = zoom(np.ones_like(beta_gt) * beta0_diff[bb], 1./grid_ms[stage])
        for st in range(stage):
            if st > 0:
                zoom_ms  = grid_ms[st - 1] / grid_ms[st] * np.ones(3)
                beta_tmp = zoom(beta_tmp, zoom_ms, mode='nearest')     
            if np.mod(np.log2(beta_tmp.shape[0]),1) == 0:
                Np_scale /= 2.
            prior_w  *= 1#4
            alpha    *= alpha_scale    
        ...
        for ii in range(max_iterations - prev_iter): 
            ...
            if (mb[ll, iteration] < min_mb):
                ...
                bb0 = beta0_diff[bb]
                sio.savemat(flags.out_path + out_name + ' min_mb.mat',
            ...
            c1_mz = np.max(beta_c.flatten()) #20.
            #c2_mz = 10.
            ...
            runtime[ll, iteration] = end - start + runtime[bb, prev_iter]
            if beta.max() > 500:
                print("ERROR")
            ...
            if (up_scale >= cost_window_size + 1) and (stage < (params.n_stages - 1)):
                ...
                sio.savemat(flags.out_path + out_name + ' start of stage ' + str(stage) + '.mat',
                            {'beta0_diff': beta0_diff[bb], 'mask': mask, 'beta_gt': beta_gt, 'params': params, #'air_p': air_p,  
                             'flags': flags,                                               # Scene pre-fixed params
                             'alpha': alpha, 'beta1': beta1, 'beta2': beta2,               # Optimization hyper-params
                             'smooth_lambda': slambda, 'smooth_term': smooth_term[ll],     # Smooth prior
            ...
            if (np.mod(iteration, 10) == 0) and (iteration > 0):
                bb0 = beta0_diff[bb]
                sio.savemat(flags.out_path + out_name + ' iter.mat',
                            {'beta0_diff': beta0_diff[bb], 'mask': mask, 'beta_gt': beta_gt, 'params': params, #'air_p': air_p,  
                             'flags': flags,                                               # Scene pre-fixed params
                             'alpha': alpha, 'beta1': beta1, 'beta2': beta2,               # Optimization hyper-params
                             'smooth_lambda': slambda, 'smooth_term': smooth_term[ll],     # Smooth prior
