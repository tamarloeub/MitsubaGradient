...
non0_x, non0_y, non0_z = np.where(mask > 0.07)
ind0f  = np.where(mask.flatten('F') <=0.07) #0) ## CORRECT MASK
non0_f = np.where(mask.flatten('F') > 0.07)
...
for ll in range(len(lambda_factor)):
    ...
    out_name = '19_07_30_16:34/smooth prior lambda ' + str(slambda) + ' full '
    out_name += 'org jpl cloud with air and ocean ' + str(grid_size) + ' grid points ' + str(n_unknowns) + ' unknowns ' + str(n_sensors)
    out_name += ' sensors all above the medium 1 cycle ' + str(n_pixels) + ' pixels ' + str(Np_vector[0]) + ' photons ' + str(alpha) + ' adam '
    out_name += 'alpha with space carving mask'
    ...
    for bb in range(len(beta0_diff)):
        ...
        cost_iter = 0
        beta      = load_res['beta']
        I_algo[:, :, :prev_iter] = load_res['I_algo'][:, :, :prev_iter]
        betas[bb, :prev_iter]     = load_res['betas'][bb, :prev_iter]
        
        if 'mb' in load_res.keys():
            mb[:prev_iter] = load_res['mb'][:,:prev_iter]
        elif 'betas' in load_res.keys():
            for ii_ in range(prev_iter):
                tmp_b  = np.reshape(betas[bb, ii_], beta_gt.shape, 'F')
                mb[ii_] = np.sum(np.sum(np.sum( abs(tmp_b - beta_gt), 2), 1), 0 ) / np.sum( beta_gt_flat ) * 100
        ...
        for ii in range(max_iterations - prev_iter): 
            ...
            mb[iteration] = np.sum(np.sum(np.sum( abs(beta - beta_gt), 2), 1), 0 ) / np.sum( beta_gt_flat ) * 100
            ...
            for ss in range(n_sensors):
                ...
                algo_scene.create_new_scene(beta=np.copy(beta), g=0.85, origin=sensors_pos[ss]['origin'], target=sensors_pos[ss]['target'],
                                            up=sensors_pos[ss]['up'], nSamples=Np_vector[0], sensorType='perspective', fov=fov_deg, 
                                            bounding_box=bounds, width=n_pixels_w, height=n_pixels_h)

                [ I_algo[ss, bb, iteration], inner_grad ] = render_scene(algo_scene._scene, output_filename, n_cores, grid_size, n_pixels_w, 
                                                                         n_pixels_h)
            ...
            cost_grad_mat = np.reshape(cost_grad, beta.shape, 'F') + smooth_grad
            ...
            if (np.mod(iteration, 50) == 0) and (iteration > 0) and (ii > 0):
                bb0 = beta0_diff[bb]
                sio.savemat(out_path + out_name + ' iter.mat', 
                            { 'beta0_diff' : beta0_diff[bb], 'mask' : mask, 'beta_gt' : beta_gt,
                              ...
                              'cost' : cost, 'I_algo': I_algo, 'betas' : betas, 'runtime' : runtime, 'mb' : mb, 
                              'iteration' : iteration, 'beta' : beta, 'costMSE' : costMSE })      # algorithm calculated variables
    
                iters = np.linspace(1, iteration, iteration) 
                plt.figure(figsize=(19,9))    
                plt.plot(iters, cost[bb,0:iteration]+ slambda * smooth_term[bb,0:iteration], '--',  marker='o', markersize=5)
